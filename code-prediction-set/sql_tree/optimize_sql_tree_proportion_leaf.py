...
def solve_optimization(tree, proportion):
    ...
    num_nodes_to_remove = np.ceil(len(map_node_name_to_include) * proportion)
    num_nodes_removed = 0
...
def create_tree_from_optimization_result(tree, tau: float):
    proportion = np.exp(-1 * tau)
    assert proportion >= 0 and proportion <= 1
    map_node_name_to_include, pruned_tree_error = solve_optimization(tree, proportion)
